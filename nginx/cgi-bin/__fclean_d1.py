...
# script_path = r'C:\Users\amanv\Desktop\DevStakes\Project_1\Task_1\Goverdhan\nginx\cgi-bin\__fclean_d1.py'
pids = []
curpos = []
endcount = 0
curserver = 0
...
# from config.serverlist_4096 import _smtpserver
import tkinter as tk
from tkinter import ttk
import threading
import time  # This is for simulation; replace with actual script logic

def run_script():
    # Simulate the main script execution
    # Replace this with your actual script's processing logic
    time.sleep(10)  # Example logic, replace with actual work
...
def start_loader():
    ...
    result_label = tk.Label(root, text="Processing, please wait...")
    result_label.pack(pady=10)

    progress_bar.start()

    # Keep the window open while the script is running
    root.after(100, lambda: check_completion(root, progress_bar, result_label))
    root.mainloop()
...
def _check_ippause(_ippause, per_swait):
    curtime = time.time()
...
def openMail(smtp_server, local_ip, mail_from):
    if not local_ip:
        print("No LOCAL IP set")
        return 1

    if not smtp_server:
        print("No SMTP Server set")
        return 1

    if not mail_from:
        print("No MAIL FROM set")
        return 1

    print(f"Opening {smtp_server} : from {local_ip}")

    # Get the port number for SMTP
    port = socket.getservbyname('smtp', 'tcp')
    ...
    try:
        ...
        s.settimeout(5)
        s.bind((local_ip, 0))  # Bind to a local IP address and port
        ...
        while response.startswith(tuple(f"{code}-" for code in range(100, 600))):
            response = s.recv(1024).decode()

        if not response.startswith(('220', '250')):
            s.close()
            print(f"HELO command failed: {response}")
            return 1

        # Send MAIL FROM command
        mail_command = f"MAIL FROM:<{mail_from}>\r\n"
        s.sendall(mail_command.encode())
        response = s.recv(1024).decode()

        if not response.startswith(('220', '250')):
            s.close()
            print(f"MAIL command failed: {response}")
            return 1
...
_check_ippause(_ippause, per['swait'])
...
def check_completion(root, progress_bar, result_label):
    # Check if the script has finished processing
    if not script_thread.is_alive():
        # Stop the progress bar
        progress_bar.stop()
        result_label.config(text="Process completed!", fg="green")
        root.after(2000, root.destroy)  # Close window after 2 seconds
    else:
        root.after(100, lambda: check_completion(root, progress_bar, result_label))

# Create and start the thread for running the script
script_thread = threading.Thread(target=run_script)
script_thread.start()

# Start the loader in the main thread
start_loader()
