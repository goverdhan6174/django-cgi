...
##----------------------------------------
## Changed 2023/11/3 => 11/9 Roll backed
$THREADS = 100 ;
#$THREADS = 50 ;
##----------------------------------------
...
#	print("[$cloop] Status($status) Beginning THREAD \n");
	&_dprint("[$cloop] Status($status) Beginning THREAD \n");
        ##----------------------------------------
	...
	##
	  ...
	  if($_OPFLAG eq 0){
		...
		$iploop=0;
          ...
          ## Added 2023/10/31(2)
          if(! $thr[$thloop]) {
            &_dprint("[$cloop][$thloop] $$ thread->new() Failed\n");
          }
          ##----------------------------------------
#	  $thr[$thloop] -> detach();
          ##----------------------------------------
...
#          print("[$cloop][$thloop] $$ Status : $status\n");
          &_dprint("[$cloop][$thloop] $$ Status : $status\n");
          ##----------------------------------------
	  #
	  ...
	  if($status eq 1 || $_dolined{$cloop} >= $_doline{$cloop} ){
            ##----------------------------------------
            ## Added 2023/10/29
            &_dprint("[$cloop][$thloop] $$ Thread next - dolined / doline : $_dolined{$cloop} / $_doline{$cloop}\n");
            ##----------------------------------------
            next;
#  	    $thloop++;
#	    last;
...
#          print("- Waiting for Thread WAIT $per{twait}\n");
          &_dprint("- Waiting for Thread WAIT $per{twait}\n");
          ##----------------------------------------
	  usleep($per{twait});
          ##----------------------------------------
          ## Changed 2023/11/2
#          print("- Waiting for Thread WAIT;End\n");	    
          &_dprint("- Waiting for Thread WAIT;End\n");	    
          ##----------------------------------------
	}
      }
      ##----------------------------------------
...
#      print "Finished: $child\n";
      &_dprint( "Finished: $child\n" );
      ##----------------------------------------
  }
  else {
      ##----------------------------------------
      ## Added 2023/10/29
      &_dprint("$$ Pushed: $child\n");
      ##----------------------------------------
      push(@pids, $child);
  }
}
...
##----------------------------------------
## Added 2023/10/29
&_dprint("$$ Sleep 60 sec before exit\n");
sleep(60);
##----------------------------------------
...
#  print("Current Status $_tcounts / $total / $totals\n");
  &_dprint("Current Status $_tcounts / $total / $totals\n");
  ##----------------------------------------
}
...
#  print("[$pnum][$thnum] $$ Opening SMTP Server : [$_OPFLAG][$_TDOMAIN][$curserver] ".$_smtpserver{$_TDOMAIN}{$curserver}." with IP [$ownip] $iplist[$ownip]\n");
  &_dprint("[$pnum][$thnum] $$ Opening SMTP Server : [$_OPFLAG][$_TDOMAIN][$curserver] ".$_smtpserver{$_TDOMAIN}{$curserver}." with IP [$ownip] $iplist[$ownip]\n");
  ##----------------------------------------
  ##
...
#      &verifyMail($_emails[$loop],$pnum,0);
      &verifyMail($_emails[$loop],$pnum,0,  $thnum,$loop);
      ##----------------------------------------
...
#  return($#_emails, $_remails);
  ...
  if ($@) {
    &_dprint("[$pnum][$thnum] $$ Exception occur: $@");
  }
  ##----------------------------------------
}
...
#  print("Opening $_smtpserver : $port : from $_localip \n");
  &_dprint("Opening $_smtpserver : $port : from $_localip \n");
  ##----------------------------------------
  ...
  ##----------------------------------------
  ## Added 2023/11/3
  &_dprint("$$ PRE $_localip / fileno : " . fileno($SCK) . "\n");
  ##----------------------------------------
...
#    LocalPort => 9001
  ...
  ##----------------------------------------
  ## Added 2023/11/3
  &_dprint("$$ PST $_localip / fileno : " . fileno($SCK) . "\n");
  ##----------------------------------------
...
sub verifyMail()
{
  ...
  ##----------------------------------------
  ## Added 2023/11/3
  my $thnum = $_[3];    # Thread Number
  my $loop  = $_[4];    # Email No.
  ##----------------------------------------
  ...
  ## Added 2023/10/21
  {
    ...
    if (($_s_emails_done % 100) == 0) {
      &_dprint( "Progress : $_s_emails_done / $_s_emails_total\n" );
    }
  }
  ##----------------------------------------
...
##  close(FP);
  ...
  if(!$SCK){
    ##----------------------------------------
    ## Added 2023/11/3
    my $mess = sprintf("[$_pnum][%2d] $$ No.%d : SCK is NULL.\n", $thnum, $loop);
    &_dprint($mess);
    ##----------------------------------------
    return;      
  }
...
#  my $timeo = pack("qq", 3, 0);
  ...
  ## Added 2023/11/3
  if (! defined $res) {
    my $mess = sprintf("[$_pnum][%2d] $$ No.%d : SCK read none. : $!\n", $thnum, $loop);
    &_dprint($mess);
  }
  ##----------------------------------------
  $res =~ s/\x0D\x0A|\x0D|\x0A/\n/g;
  ...
  ## Changed 2023/11/3
  #print($res."\n");
  my $mess = sprintf("[$_pnum][%2d] $$ No.%d : $res.\n", $thnum, $loop);
  &_dprint($mess);
  ##----------------------------------------
  while($res =~ m/^\d{3}\-/){
    $res = <$SCK>;
...
sub _dprint()
{
  ...
  if ($_curdate eq "") {
    $year += 1900;
    $mon += 1;
    $_curdate = sprintf("%04d%02d%02d", $year, $mon, $mday );
  }
  my $_curtime = sprintf("%02d/%02d %02d:%02d:%02d  ", $mon, $mday, $hour, $min, $sec );

  open(FPD, ">> /tmp/debug_output_${ipv}_${_curdate}.txt");
  print FPD $_curtime . $message;
  close(FPD);

  print $_curtime . $message;
}
##----------------------------------------
## Added 2023/11/2
sub _sigpipe
{
  &_dprint("Progress : SIGPIPE\n");
}
##----------------------------------------
